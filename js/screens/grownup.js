...
/* EVERY STORED STRING ON THIS SCREEN GOES THROUGH HERE.
   ...
   The Progress screen has done this for a while; this one had not, and it is
   the screen that renders the most stored text — session titles, exercise
   names off the ledger, prize labels, the athlete's name, flag and stop-note
   narrative. All of it can arrive from a restored backup or from the cloud
   mirror, which is unauthenticated by an explicit owner decision (see the
   comments at the top of firestore.rules — that file names rendering stored
   text AS TEXT as the app's defence, and until now only half the app kept it).

   Applied to static strings too, where it is a no-op. A rule with exceptions is
   a rule somebody has to remember; this one is just "escape it". */
  ...
  <div style="background:var(--surface);border:2px solid var(--sun);border-radius:var(--radius-xl);padding:16px 18px;box-shadow:var(--shadow-soft);">
    ...
    ${a.flags.map(fl => `
      <div style="${fl.rowStyle}">
        <span style="font-size:18px;line-height:1;flex-shrink:0;">${esc(fl.icon)}</span>
        <span style="font-size:14px;font-weight:700;color:var(--ink);line-height:1.4;">${esc(fl.text)}</span>
      </div>`).join("")}
  </div>` : "";
}
...
function stopsCard(a, scopeLabel) {
    ...
    ${a.stopEvents.map(se => `
      <div style="display:flex;gap:10px;align-items:flex-start;margin-top:10px;">
        ...
        <div style="flex:1;min-width:0;">
          <div style="font-size:14px;font-weight:900;color:var(--ink);">${esc(se.date)} · ${esc(se.move)}</div>
          <div style="font-size:13px;font-weight:700;color:var(--ink-soft);line-height:1.4;margin-top:2px;">${esc(se.note)}</div>
        </div>
      </div>`).join("")}
  </div>` : `
  <div style="display:flex;align-items:center;gap:10px;background:var(--mint-wash);border-radius:var(--radius-xl);padding:13px 18px;">
    ...
    <span style="font-size:14px;font-weight:800;color:var(--mint-ink);">No stop-rule events · ${esc(scopeLabel)} — the stop rule wasn't needed. ✓</span>
  </div>`;
}
  ...
  <div style="display:flex;flex-direction:column;gap:14px;">
    ...
    <div style="background:var(--surface);border:2px solid var(--aqua-light);border-radius:var(--radius-xl);padding:16px 18px;box-shadow:var(--shadow-soft);">
      ...
      ${vm.guAlerts.map(ga => `
        <div style="${ga.rowStyle}">
          <span style="font-size:18px;line-height:1;flex-shrink:0;">${esc(ga.icon)}</span>
          <span style="font-size:14px;font-weight:700;color:var(--ink);line-height:1.4;">${esc(ga.text)}</span>
        </div>`).join("")}
    </div>
    ...
    <div style="background:var(--aqua-wash);border-radius:var(--radius-xl);padding:16px 18px;display:flex;gap:12px;align-items:flex-start;">
      ...
      <div>
        ...
        <div style="font-size:15px;font-weight:700;color:var(--ink);line-height:1.5;margin-top:3px;">${esc(a.read)}</div>
      </div>
    </div>
    ${card(`
  ...
  <div style="display:flex;flex-direction:column;gap:14px;">
    ...
    ${card(`
      <div style="display:flex;align-items:center;gap:16px;flex-wrap:wrap;">
        ...
        <div style="flex:1;min-width:220px;font-size:13px;font-weight:700;color:var(--ink-soft);line-height:1.45;">${esc(a.acwr.note)}</div>
      </div>`)}
    ...
    <div style="${vm.grid2}">
      ...
      ${card(`
        ...
        <div style="display:flex;flex-direction:column;gap:10px;">
          ${a.skips.map(sk => `
            <div style="display:flex;align-items:center;gap:10px;">
              <span style="width:96px;font-size:14px;font-weight:700;color:var(--ink);flex-shrink:0;">${esc(sk.label)}</span>
      ...
      <div style="display:flex;flex-direction:column;gap:8px;">
        ${a.byWeekday.map(wd => `
          <div style="display:flex;align-items:center;gap:12px;background:${wd.rowBg};border-radius:12px;padding:10px 13px;">
            <span style="width:36px;font-size:13px;font-weight:900;color:var(--ink);flex-shrink:0;">${esc(wd.k)}</span>
            <span style="flex:1;min-width:0;font-size:14px;font-weight:700;color:var(--ink);white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">${esc(wd.topic)}</span>
            <span style="font-size:20px;flex-shrink:0;">${esc(wd.mood)}</span>
            <span style="${wd.statusStyle}">${esc(wd.statusChip)}</span>
          </div>`).join("")}
      </div>`) : ""}
    ${a.byTopic ? card(`
        ...
        ${a.byTopic.map(tp => `
          <div>
            <div style="display:flex;align-items:center;gap:10px;margin-bottom:5px;">
              <span style="flex:1;min-width:0;font-size:14px;font-weight:800;color:var(--ink);white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">${esc(tp.k)}</span>
              <span style="font-size:18px;flex-shrink:0;">${esc(tp.mood)}</span>
              <span style="font-size:13px;font-weight:900;color:var(--ink-soft);flex-shrink:0;width:64px;text-align:right;">${esc(tp.ratio)} · ${tp.pct}%</span>
            </div>
    ...
    <div style="${vm.grid2}">
      ...
      <div style="background:var(--mint-wash);border-radius:var(--radius-xl);padding:16px 18px;display:flex;gap:12px;align-items:flex-start;">
        ...
        <div>
          ...
          <div style="font-size:15px;font-weight:700;color:var(--ink);line-height:1.5;margin-top:3px;">${esc(a.suggest)}</div>
        </div>
      </div>
    </div>
  </div>`;
  ...
  <div style="display:flex;flex-direction:column;gap:14px;">
    ...
    ${card(`
      ...
      <div style="display:flex;flex-direction:column;gap:10px;">
        ${c.ladderRows.map(r => `
          <div style="display:flex;align-items:center;gap:10px;flex-wrap:wrap;">
            <span style="flex:1;min-width:180px;font-size:14px;font-weight:800;color:var(--ink);">${esc(r.name)}</span>
        ...
        ${Object.entries(c.engagementSystems).map(([key, sys]) => `
          <button type="button" data-action="pickEngagement" data-arg="${key}" style="display:flex;align-items:flex-start;gap:10px;text-align:left;background:${c.engagement === key ? "var(--aqua-wash)" : "var(--surface-2)"};border:2px solid ${c.engagement === key ? "var(--aqua)" : "transparent"};border-radius:14px;padding:12px 14px;cursor:pointer;font-family:inherit;">
            ...
            <span>
              ...
              <span style="display:block;font-size:13px;font-weight:700;color:var(--ink-soft);margin-top:2px;">${esc(sys.desc)}</span>
            </span>
          </button>`).join("")}
      </div>`)}
  </div>`;
